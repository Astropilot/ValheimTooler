...
namespace ValheimTooler.Configuration
{
	...
	public static class TomlTypeConverter
    {
        ...
        public static bool AddConverter(Type type, TypeConverter converter)
        {
            ...
            if (CanConvert(type))
            {
                ZLog.LogWarning("Tried to add a TomlConverter when one already exists for type " + type.FullName);
                return false;
            }
        ...
        {
            try
            { LazyTomlConverterLoader.AddUnityEngineConverters(); }
            catch (Exception ex) { ZLog.LogWarning("Failed to load UnityEngine Toml converters - " + ex.Message); }
        }
